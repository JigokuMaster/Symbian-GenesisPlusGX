...
#include <sdlexe.rsg>
#else
#include <GenesisPlusGX.rsg>
#include "GenesisPlusGX.hrh"
#endif
...
LOCAL_C void SetEPOCEnvVars()
{
    ...
    sprintf(privDir, "%c:\\private\\%08x\\", d, uid);
...
#else
	BaseConstructL(EAknEnableSkin | ENoScreenFurniture);
	TFileName name(_L("\\resource\\apps\\GenesisPlusGX.mbm"));
#endif		
	//TEntry e;
...
TBool CSDLAppUi::ProcessCommandParametersL(CApaCommandLine &aCommandLine)
 ...
 void CSDLAppUi::StartL()	
	...
	//iParams->AppendL(_L8("C:\\Data\\GenesisPlusGX\\sonic.sms"));
 	iWait = new (ELeave) CExitWait(*this);
	if ( ApplicationRect().Width() == 360 )
	    {
...
#endif
    return SDLEnv::SetMain(SDL_main, CSDL::EEnableFocusStop /*| CSDL::EImageResizeZoomOut*/ | CSDL::EAllowImageResizeKeepRatio, NULL, exe_flags);
}
